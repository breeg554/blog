...
When I was sitting out to create my own blog, I spent quite a long time thinking - how to handle post and where to store my posts.

Should I use headless CMS like [Contentful](https://www.contentful.com/) or maybe the better option would be to managing mdx files and parsing them using, for example, [remark](https://www.npmjs.com/package/remark)?
The second option seemed very good for me, also because I saw an entire section dedicated to this on Next.js [documentation](https://nextjs.org/learn/basics/dynamic-routes/render-markdown), so the setup should be very easy.

However, once upon a time, when I was browsing my Twitter bubble, I came across a tweet from [@delba_oliveira](https://twitter.com/delba_oliveira) where she described the new Next.js documentation stack.

<StaticTweet id="1658525395988119569" />

As you can see, MDX files are processed by [Contentlayer](https://www.contentlayer.dev/). After reviewing the documentation, I decided to try it out on my own blog. I can truly say now that it was a great decision.
The handling of MDX files, types generation, data transformation and easy integration with Next.js App Directory - all of this makes the dev experience soo good that I decided to write a blog about it 🚀

Ok, but let's take it step by step. What will this post be about? It won't be a tutorial or something like that. I just want to show you how easy the integration between Next and Contentlayer is
and how easy you can do it by yourself. If you wonder whether you use Contentlayer or not, maybe this post will help you make decision.
...
## Defining schema. No way that is soo easy
...
At the bottom of the file, you can see invocation of **makeSource** function, which provide content and schema for our app.
There is **contentDirPath option which points to the folder where our posts are stored** (in my case it's content directory).
...
I also wanted to add a unique field for each post that I could use in the URL. There was an option to add additional field to each post, for example, "slug",
or use what I already have, which is name file name, right? (which I already have in **_raw.flattenedPath**).
...
However, what if we would like to customize our h2 element and show anchor after hovering? (try it out on the heading above)
Nice, isn't it? Well, to achieve this, we can use our hook again, but additionally, we have to pass the prop with custom components:
...
So, as you can see, the setup itself is quite easy, as are the possibilities that Contentlayer gives us. Of course, that was just a basic setup - you can do much, much more.

I hope, I've helped you make a decision if you were wondering if Contentlayer is a good choice. If so, let my know by hitting me up on Twitter!
